...
import com.example.findhospital.data.source.TasksRepository
...
class TasksViewModel(
    ...
    private fun Event(unit: String): Event<Unit>? {
        return Event("Kotlin.Unit")
    }
    ...
    fun showEditResultMessage(result: Int) {
        ...
        val EDIT_RESULT_OK = 0
        val ADD_EDIT_RESULT_OK = 1
        val DELETE_RESULT_OK = 2
        when (result) {
    ...
    private fun filterTasks(tasksResult: Result<List<Task>>): LiveData<List<Task>> {
        ...
        if (tasksResult is com.example.findhospital.data.Result.Success<*>) {
            isDataLoadingError.value = false
            viewModelScope.launch {
                result.value = filterItems(tasksResult.data as List<Task>, getSavedFilterType())
            }
        } else {
